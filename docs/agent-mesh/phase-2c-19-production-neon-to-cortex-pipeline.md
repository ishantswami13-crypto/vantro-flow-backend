...
2. **Terminal 1:** `node scripts/phase-2c-19-launch-staging-sidecar.js` — secret-safe **hardened** launcher: loads `.env.staging` silently, **builds the `aws-1-ap-southeast-1` session-pooler `DATABASE_URL` in memory** from the staging ref+password (the direct host is IPv6-only; URL/secret never printed or written; generation/region allowlist-validated; **fail-closes** on a direct/non-pooler or production host), resolves `JWT_SECRET`, sets `RUST_AUTOMATION_API_ENABLED=true`/`RUST_AUTOMATION_PORT=3002`, spawns `target/debug/vantro-automation.exe`, and emits `sidecar_listening` (booleans/labels only).
...
| Branch / commit | PR head `phase-2c-19-owner-briefing-evidence-gate`; proof commit `8c85c85` (base `performance-bootstrap-cortex-fix-v1` @ `b3fb251`), hardened by this PR's `fix(phase-2c-19)` commit |
...
Method: the gate applies the authoritative `enforceEvidenceContract` (production code, not a copy) and cross-checks each evidence `source_id` against OWNER_A's real Cortex row universe via read-only staging REST.

**Hardening (post-Codex review, 2026-06-08; this PR's `fix(phase-2c-19)` commit).** Codex found that the committed launcher carried **no pooler logic** (the working `aws-1` construction had lived only in a since-deleted temp wrapper, so the launcher alone could not reproduce the staging connection) and that the gate's pass conditions were too soft. Both are fixed in-tree and the proof was rerun:
- **Launcher** now builds the `aws-1-ap-southeast-1` session-pooler URL **in memory** from the staging ref+password (generation/region allowlist-validated; the IPv6-only direct host **fail-closes**; URL/secret never printed or written) and emits `sidecar_listening`. Verified **standalone** (no wrapper): `host_type=pooler · pooler_generation=aws-1 · region=ap-southeast-1 · production_blocked=true · binary_found=true · sidecar_listening=true`.
- **Gate** now *requires* (fail-closed — no "unknown but pass"): OWNER_A `claims_count>0` + `safe_claim_count>0`, `evidence_subset_ok` (OWNER_A row universe must be checkable), `evidence_includes_synced`, `no_raw_customer_id`; OWNER_B `claims_count===0` + `no_owner_a_data` + `safe_to_show===false`; plus `subset_check_available`, `synced_set_available`, `production_endpoint_blocked`, `preview_path_is_readonly`, `rest_prod_blocked`.
- **Hardened rerun (UTC `2026-06-08T15:42Z`) → `overall_pass=true`:** OWNER_A `200 · safe_to_show=true · evidence=12 · claims=2 (2 safe) · subset_ok=true · includes_synced=true · no raw customer_id`; OWNER_B `200 · safe_to_show=false · evidence=0 · claims=0 · isolation_ok=true · no_owner_a_data=true`. No production/Railway/env/secret exposure; read-only `/preview` only.
