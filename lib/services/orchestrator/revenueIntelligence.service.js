...
async function computeCustomerPortfolioEntry(userId, { customerId, customerName }, opts = {}) {
  ...
  const { classifyScoreTrajectory, countRecentTierChanges } = require('../agents/creditRiskAgent');
  const trajectory = classifyScoreTrajectory(historyRows?.data || []);
  // Sustained-deterioration reuse (§11 item 3) requires CREDIT_RISK_TIER_CHANGED
  // events; when unavailable in this call path we conservatively treat trajectory
  // alone (without the sustained qualifier) as non-sustained, matching the
  // documented "never invent a stronger claim than the evidence supports" rule.
  const sustainedDeterioration = false;
