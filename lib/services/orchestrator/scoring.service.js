...
async function recalculate(userId, customerId) {
  ...
  try {
    ...
    // Read the current persisted row BEFORE overwriting it, so we can tell whether
    // this recalculation actually changed anything (Phase 3B: avoid duplicate
    // history rows on a no-op recalculation — recalculate() is called both on a
    // cron and event-triggered per §6/§13 of the Phase 3 plan, so a naive
    // "insert every call" policy would produce many identical rows per day).
    const { data: priorRow } = await supabase
      .from('customer_scores')
      .select('credit_risk_score, promise_reliability_score, broken_promise_count, collection_priority_score')
      .eq('user_id', userId)
      .eq('customer_id', customerId)
      .maybeSingle();
    ...
    // Phase 3B: append-only history, one row per materially-changed recalculation.
    // "Materially changed" = any of the 4 snapshotted columns differs from the
    // prior persisted value (or there was no prior row at all — first score ever
    // recorded for this customer is itself a change worth capturing).
    const changed = !priorRow
      || Number(priorRow.credit_risk_score)         !== Number(payload.credit_risk_score)
      || Number(priorRow.promise_reliability_score) !== Number(payload.promise_reliability_score)
      || Number(priorRow.broken_promise_count)       !== Number(payload.broken_promise_count)
      || Number(priorRow.collection_priority_score) !== Number(payload.collection_priority_score);

    if (changed) {
      const { error: histError } = await supabase.from('customer_score_history').insert([{
        user_id:                    userId,
        customer_id:                customerId,
        credit_risk_score:          payload.credit_risk_score,
        promise_reliability_score:  payload.promise_reliability_score,
        broken_promise_count:       payload.broken_promise_count,
        collection_priority_score:  payload.collection_priority_score,
      }]);
      if (histError) {
        // Never let history-write failure break the live score write it's derived from.
        safeLog('warn', '[ScoringService] History insert failed', { error: histError.message, customerId, userId });
      }
    // Record the change in the event store. business_events is described as the
    // single source of truth for what happened and when, and a customer's risk
    // profile changing is exactly that. cortex-lab's broken-promise scenario has
    // asserted CUSTOMER_RISK_UPDATED since it was written, but nothing emitted
    // it — the scoring ran and left no trace.
    //
    // Emitted here rather than at each caller so every path that recalculates is
    // covered by one line. Persist-only: no rule subscribes to this type, so it
    // adds a row and changes no behaviour. emit() never throws and returns null
    // on failure, so a failed write cannot break scoring.
    if (isEnabled('cortex_enabled')) {
      require('./event.service').emit(userId, {
