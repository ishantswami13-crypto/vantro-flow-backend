...
function getHandlers() {
  ...
  _handlers = {
    ...
    //
    // `testMode` (REWORK FIX, follow-up to d1b0ead): when the real-send guard
    // is closed (flag off / not configured), the DEFAULT behavior is to throw
    // NOT_CONFIGURED with zero mutation — identical to this route's original,
    // pre-commandBus behavior. Falling through to the fake test adapter is
    // ONLY reachable when the caller explicitly passes `testMode: true`. This
    // parameter is never derived from public HTTP request input (see
    // server.js: the send-whatsapp route never reads it from req.body/query),
    // so a real API caller cannot self-report "test mode" to get a fabricated
    // success. It exists purely for internal/dev callers — this phase's own
    // test suite (scripts/test-phase15-execution-wiring.js) and any future
    // explicit dev/staging tooling that calls this handler or a service
    // function directly.
    //
    // Honesty of test-mode completions (chose option "b" — bounded fix, no
    // schema change): ai_actions.status has a DB CHECK constraint limited to
    // ('pending','approved','rejected','done','expired','system_blocked')
    // (migration 001), so introducing a new status value (e.g. 'done_test')
    // would require a migration, which is out of scope here. Instead, the
    // authoritative signal that a 'done' action was NOT a real send is
    // execution_records.channel = 'test' (already recorded, migration 014).
    // FLAG FOR PHASE D (outcome verification): any later evaluation/learning
    // loop that treats ai_actions.status = 'done' as "a real outcome to
    // verify against payment data" MUST first join execution_records on
    // ai_action_id and exclude/handle rows where channel = 'test' separately
    // — otherwise a testMode:true completion will be indistinguishable from a
    // real one to that code, exactly as the original reviewer finding warned.
    EXECUTE_RECEIVABLES_ACTION: async (userId, { actionId, idempotencyKey, realSender, testMode } = {}) => {
      ...
      if (useRealChannel) {
        ...
        // Explicit, deliberate opt-in only — `testMode` must never be derived
        // from public HTTP request input (see server.js route: it is never
        // read from req.body/req.query). It exists solely so this phase's own
        // test suite (scripts/test-phase15-execution-wiring.js) and future
        // internal/dev tooling can exercise the execution-wiring logic
        // end-to-end without real Twilio credentials, without that behavior
        // being silently reachable by a normal caller.
        channel = 'test';
        ...
        // Real sending isn't authorized (flag off) or isn't configured, and
        // this is NOT an explicit test invocation — this is the default,
        // automatic path a normal API caller hits. Restore the original
        // pre-d1b0ead behavior byte-for-byte: an honest "not configured"
        // error, no execution_records row, no ai_actions mutation. Silently
        // falling through to the fake test adapter here (the prior bug) would
        // fabricate a 'done' outcome that evaluationAgent.js would later
        // treat as a real, verifiable completion.
        const e = new Error('WhatsApp not configured — set TWILIO_WHATSAPP_NUMBER in Railway');
        e.code = 'NOT_CONFIGURED';
        throw e;
      }
