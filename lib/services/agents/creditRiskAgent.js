...
async function run(userId, context = {}) {
  try {
    ...
    const { data: scoresRaw, error } = await supabase
      ...
      .select('customer_id, credit_risk_score, score_reason_json')
      .eq('user_id', userId)
      .order('credit_risk_score', { ascending: false });

    if (error) throw error;
    if (!scoresRaw?.length) return [];

    // The local pg-backed shim doesn't parse PostgREST embed syntax
    // (`customers(name)`), so fetch names via a bulk follow-up query instead
    // of an embedded select — tenant-scoped, one query, not per-row.
    const scoreCustomerIds = [...new Set(scoresRaw.map(s => s.customer_id).filter(Boolean))];
    let customerNameById = {};
    if (scoreCustomerIds.length) {
      const { data: customerRows } = await supabase
        .from('customers')
        .select('id, name')
        .eq('user_id', userId)
        .in('id', scoreCustomerIds);
      customerNameById = Object.fromEntries((customerRows || []).map(c => [c.id, c.name]));
    }
    const scores = scoresRaw.map(s => ({ ...s, customers: { name: customerNameById[s.customer_id] } }));
