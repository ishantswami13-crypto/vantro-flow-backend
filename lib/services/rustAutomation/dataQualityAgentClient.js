...
function validateShape(json) {
  // Accept both snake_case (Node fallback format) and camelCase (Rust API format).
  const agentId      = json.agent_id      ?? json.agentId;
  const totalFindings = json.total_findings ?? json.totalFindings;
  if (typeof agentId !== 'string')         return 'missing agent_id / agentId';
  if (typeof totalFindings !== 'number')   return 'missing total_findings / totalFindings';
