...
function makeShim(databaseUrl) {
  ...
  class QueryBuilder {
    ...
    // .not(column, operator, value): negates an arbitrary postgrest-style operator.
    // `operator` uses postgrest naming (eq/neq/gt/gte/lt/lte/like/ilike/is/in/cs), same
    // vocabulary supabase-js accepts, translated here to the SQL operator this shim uses
    // internally for the equivalent non-negated method.
    not(col, op, val) {
      const map = {
        eq: '=', neq: '!=', gt: '>', gte: '>=', lt: '<', lte: '<=',
        like: 'LIKE', ilike: 'ILIKE', is: 'IS', in: 'IN', cs: '@>',
      };
      const sqlOp = map[String(op).toLowerCase()];
      if (!sqlOp) throw new Error(`pgSupabaseShim: .not() does not support operator "${op}"`);
      this._filters.push({ col, op: 'NOT', innerOp: sqlOp, val });
      return this;
    }
    ...
    _whereClause(startIdx) {
      ...
      for (const f of this._filters) {
        if (f.op === 'IN') {
          ...
          if (f.innerOp === 'IN') {
            clauses.push(`NOT (${quoteIdent(f.col)} = ANY($${i++}))`);
            params.push(f.val);
          } else if (f.innerOp === 'IS') {
            clauses.push(`${quoteIdent(f.col)} IS NOT ${f.val === null ? 'NULL' : f.val}`);
          } else {
            clauses.push(`NOT (${quoteIdent(f.col)} ${f.innerOp} $${i++})`);
            params.push(f.val);
          }
        } else {
