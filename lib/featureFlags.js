...
//   FEATURE_ATLAS_AGENT_REGISTRY_API_ENABLED — expose GET /api/atlas/agents (Phase 2C.28, DEFAULTS OFF)
//   FEATURE_ATLAS_RELATIONSHIP_GRAPH_API_ENABLED — expose GET /api/atlas/relationship-graph (Phase 2C.29, DEFAULTS OFF)
//   FEATURE_ATLAS_ACTION_APPROVAL_API_ENABLED — expose GET /api/atlas/action-approvals (Phase 2C.30, DEFAULTS OFF)
//   FEATURE_CUSTOMER_REVENUE_INTELLIGENCE — revenue/momentum/concentration/dormancy/health on customer endpoints (Phase 10, DEFAULTS OFF)
...
const FLAGS = {
  ...
  tally_import_enabled:            process.env.FEATURE_TALLY_IMPORT_ENABLED               === 'true',

  // STARLANE Phase 10 — Customer & Revenue Intelligence V1. Additive-only,
  // read-only, deterministic (see revenueIntelligence.service.js). Defaults OFF.
  customer_revenue_intelligence:   process.env.FEATURE_CUSTOMER_REVENUE_INTELLIGENCE      === 'true',

  // STARLANE World Intelligence Backbone — Phase 1. Gates the USGS
  // earthquake + FX rate ingestion crons registered in server.js. Defaults
  // OFF so a bad external source or schema issue can be killed without a
  // deploy (matches every other ingestion-style flag in this file).
  world_intelligence_enabled:      process.env.FEATURE_WORLD_INTELLIGENCE_ENABLED         === 'true',
  ...
  // Atlas Agent Registry — Phase 2C.28: read-only "Agent Universe" truth.
  // Exposes GET /api/atlas/agents and GET /api/atlas/agents/:id. Static registry only.
  // No DB, no mutations, no execution, no activation, no production enablement, no
  // external send. is_implemented/harness_verified are independent facts only.
  // Counts/booleans/status/labels only. Defaults OFF — flip
  // FEATURE_ATLAS_AGENT_REGISTRY_API_ENABLED=true to expose the read-only endpoints.
  atlas_agent_registry_api_enabled: process.env.FEATURE_ATLAS_AGENT_REGISTRY_API_ENABLED === 'true',

  // Atlas Relationship Graph — Phase 2C.29: read-only topology over Pack/Agent/Workflow.
  // Exposes GET /api/atlas/relationship-graph and /relationship-graph/nodes/:id. Static
  // graph only (59 nodes, 2 stored edge types). No DB, no mutations, no execution, no
  // activation, no production access, no external send, no recursion, no /paths, no
  // caller-controlled depth. Counts/booleans/status/labels only. Defaults OFF — flip
  // FEATURE_ATLAS_RELATIONSHIP_GRAPH_API_ENABLED=true to expose the read-only endpoints.
  atlas_relationship_graph_api_enabled: process.env.FEATURE_ATLAS_RELATIONSHIP_GRAPH_API_ENABLED === 'true',

  // Atlas Action Approval Contract — Phase 2C.30: read-only approval-REQUIREMENT truth.
  // Exposes GET /api/atlas/action-approvals and /action-approvals/:id. Static registry
  // only (12 action-class contracts). Describes what approval WOULD be required; it does
  // NOT request/grant/deny/record/queue/execute approvals. No DB, no mutations, no
  // execution, no activation, no production access, no external send, no auto-approval.
  // Defaults OFF — flip FEATURE_ATLAS_ACTION_APPROVAL_API_ENABLED=true to expose the
  // read-only endpoints.
  atlas_action_approval_api_enabled: process.env.FEATURE_ATLAS_ACTION_APPROVAL_API_ENABLED === 'true',

  // Starlane Brain — read-only business intelligence summary.
  // Exposes GET /api/brain/summary. Aggregates existing invoices/transactions/
  // products/stock_movements into sales direction, gross profit, cash flow,
  // receivables/payables, smart set-off, product movement, and ranked actions.
  // Pure read + compute: no mutations, no external send, no LLM. Tenant-scoped
  // by user_id. Defaults OFF — flip FEATURE_BRAIN_DASHBOARD_ENABLED=true to expose.
  brain_dashboard_enabled:         process.env.FEATURE_BRAIN_DASHBOARD_ENABLED             === 'true',
};
