...
// Queries world_sources rows and evaluates each for freshness.
// Pure DB read + deterministic comparison — no external calls, no LLM.
// Logs a safeLog('warn', ...) per STALE/NEVER_SUCCEEDED source found.
//
// Phase 3C: excludes is_internal=true rows (test fixtures like TEST_SOURCE,
// dev/demo reference sources like manual_reference/2xa_demo_events) by
// default — those were never meant to be measured against a production
// refresh cadence, and the Sources UI / /api/world/health should never show
// "TEST_SOURCE: STALE" to an owner. This replaces the old approach (a
// hardcoded REAL_WORLD_PROVIDERS allowlist in the frontend) with an explicit
// column any caller can rely on. Pass { includeInternal: true } for
// diagnostics that genuinely need to see fixture rows too.
async function checkSourceFreshness({ includeInternal = false } = {}) {
  const pool = getPool();
  const res = await pool.query(
    `SELECT id, provider, dataset, update_cadence, last_successful_ingestion_at,
            last_failure_at, last_failure_reason, is_internal
     FROM world_sources
     ${includeInternal ? '' : 'WHERE is_internal = false'}
