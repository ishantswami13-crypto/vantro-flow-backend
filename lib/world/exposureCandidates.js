...
async function verifyCandidate(userId, candidateId, { kind = 'country', reviewedByUserId = null, reviewNotes = null } = {}) {
  ...
  // A GSTIN candidate's raw_value is the tax-ID string itself, not a
  // country-shaped value — resolveCountryValue can't (and must not) parse a
  // GSTIN. The deterministic fact it encodes is "registered in India" (see
  // extractFromGstin's header comment), so promotion resolves the literal
  // country name "India" rather than attempting to normalize the GSTIN
  // string, while still preserving the original GSTIN in evidence_notes/
  // raw_value context via provenance_reference.
  const valueToResolve = candidate.extraction_method === 'gstin_implies_india' ? 'India' : candidate.raw_value;
