...
// ISO 3166-1 alpha-3 -> alpha-2, deterministic lookup only. Only countries
// this codebase is already confident about (i.e. already present in
// COUNTRY_NAME_TO_ISO2 above) are listed here, so "VNM" and "Viet Nam"
// resolve to the exact same canonical country entity (VN). Deliberately NOT
// exhaustive — a country whose alpha-3 code isn't certain is left out rather
// than guessed, per the "no guessing" rule for this resolver.
const COUNTRY_ISO3_TO_ISO2 = {
  IND: 'IN', USA: 'US', CHN: 'CN', JPN: 'JP', DEU: 'DE', GBR: 'GB', FRA: 'FR',
  NPL: 'NP', ESP: 'ES', VNM: 'VN', IDN: 'ID', BRA: 'BR', MEX: 'MX', CAN: 'CA',
  AUS: 'AU', RUS: 'RU', KOR: 'KR', PRK: 'KP', ITA: 'IT', NLD: 'NL', SGP: 'SG',
  BGD: 'BD', PAK: 'PK', THA: 'TH', MYS: 'MY', PHL: 'PH', CHL: 'CL', PER: 'PE',
  COL: 'CO', TUR: 'TR', AFG: 'AF', CHE: 'CH', SWE: 'SE', NOR: 'NO', DNK: 'DK',
  FIN: 'FI', POL: 'PL', AUT: 'AT', BEL: 'BE', PRT: 'PT', IRL: 'IE', GRC: 'GR',
  CZE: 'CZ', HUN: 'HU', ROU: 'RO', UKR: 'UA', EGY: 'EG', ZAF: 'ZA', NGA: 'NG',
  KEN: 'KE', SAU: 'SA', ARE: 'AE', ISR: 'IL', IRN: 'IR', IRQ: 'IQ', QAT: 'QA',
  KWT: 'KW', NZL: 'NZ', TWN: 'TW', HKG: 'HK', MAC: 'MO', LKA: 'LK', MMR: 'MM',
  KHM: 'KH', LAO: 'LA', ARG: 'AR', ECU: 'EC', VEN: 'VE', BOL: 'BO', URY: 'UY',
  PRY: 'PY', CRI: 'CR', PAN: 'PA', GTM: 'GT', NIC: 'NI', ETH: 'ET', MAR: 'MA',
  DZA: 'DZ', TUN: 'TN', GHA: 'GH', ISL: 'IS', HRV: 'HR', SRB: 'RS', SVK: 'SK',
  SVN: 'SI', BGR: 'BG', KAZ: 'KZ', UZB: 'UZ', MNG: 'MN', FJI: 'FJ', PNG: 'PG',
  TON: 'TO', SLB: 'SB', VUT: 'VU',
};
...
async function resolveCountryValue(rawValue) {
  ...
  // 1.5. exact ISO 3166-1 alpha-3 code, mapped to the same canonical alpha-2
  // entity so alpha-3 input resolves identically to alpha-2/name input.
  if (/^[a-z]{3}$/i.test(rawValue.trim())) {
    const alpha3 = rawValue.trim().toUpperCase();
    const iso2 = COUNTRY_ISO3_TO_ISO2[alpha3];
    if (iso2) {
      const id = await upsertCountry(iso2);
      return { worldEntityId: id, code: iso2, method: 'alpha3_code_lookup', confidence: 0.95, normalized };
    }
  }
