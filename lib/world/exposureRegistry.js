  ...
  const res = await pool.query(
    `INSERT INTO business_exposure
     ...
     VALUES ($1,$2,$3,$4,$5, COALESCE($6, NOW()), $7, $8, $9,
             $10,$11,$12::text,$13, CASE WHEN $12::text IS NOT NULL THEN NOW() ELSE NULL END,
...
async function supersedeExposure(userId, exposureId, newValues, { supersededAt = null } = {}) {
  ...
  // The cutoff for closing the OLD row must equal the new row's valid_from
  // whenever the caller supplies one — otherwise the two rows' validity
  // windows overlap and a temporal "as of" query would incorrectly match
  // both. Precedence: explicit newValues.validFrom > explicit supersededAt
  // > now.
  const cutoff = newValues.validFrom || supersededAt || new Date().toISOString();
