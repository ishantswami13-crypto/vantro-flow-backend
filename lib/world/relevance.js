...
// Phase 3A: only VERIFIED exposures may ever produce a signal. An
// UNVERIFIED/REJECTED/SUPERSEDED row must never reach the matcher — this is
// the mandatory "prefer verified" default (see
// STARLANE_WORLD_INTELLIGENCE report, Phase 3A test suite). Candidates living
// in business_exposure_candidates are a wholly separate table and are never
// read here at all.
async function loadExposuresForTenant(userId) {
  const pool = getPool();
  const res = await pool.query(
    `SELECT * FROM business_exposure WHERE user_id = $1 AND verification_status = 'VERIFIED'`,
    [userId]
  );
  return res.rows;
}
