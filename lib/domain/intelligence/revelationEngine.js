...
const { lifecycleOf, SIGNAL_LIFECYCLE } = require('../../world/externalSignal');
const { buildIssueKey, reportIssueCheckpoint } = require('./issueLifecycle');
...
// STARLANE WORLD INTELLIGENCE EXPANSION — Part 34/35: World -> Morning
// Revelation integration.
//
// Surfaced ONLY when ALL of:
//   1. A real/seeded world exposure -> event chain exists for the tenant's
//      top-spend supplier (reuses worldEventConsequence.widenSupplierUncertaintyFromEvent
//      — no new detection logic invented here).
//   2. Materiality is non-trivial: widenSupplierUncertaintyFromEvent already
//      downgrades LOW-materiality exposures to NO_EFFECT (Part 7), so
//      requiring impact_mode === RANGE_WIDENING here is sufficient to
//      enforce "materiality non-trivial" without recomputing it.
//   3. The underlying event signal is fresh (lifecycleOf === ACTIVE) — a
//      STALE/EXPIRED/RESOLVED event does not get surfaced as new risk.
//   4. issueLifecycle's stable-identity pattern means an unchanged repeat
//      call (status transitions to STABLE, not DETECTED/WORSENING/RECURRED)
//      is NOT re-surfaced as if it were new.
//
// Per Part 35: every such revelation states explicit confirm/invalidate
// conditions.
async function revelationFromWorldExposure(userId, supplierConcentration) {
  if (!supplierConcentration || supplierConcentration.insufficientData || !supplierConcentration.top || supplierConcentration.top.length === 0) {
    return null;
  }
  const top = supplierConcentration.top[0];
  let widened;
  try {
    widened = await widenSupplierUncertaintyFromEvent({ userId, supplierId: top.supplierId });
  } catch (e) {
    safeLog('warn', '[RevelationEngine] world exposure check failed', { userId, error: e.message });
    return null;
  }
  if (widened.impact_mode !== 'RANGE_WIDENING' || !widened.chains || widened.chains.length === 0) return null;

  const chain = widened.chains[0];
  const eventObservedAt = chain.EVENT?.observed_at || null;
  const lifecycle = lifecycleOf({ observed_at: eventObservedAt });
  if (lifecycle !== SIGNAL_LIFECYCLE.ACTIVE) return null; // stale/expired/resolved signal — not surfaced as fresh risk

  const issueKey = buildIssueKey('WORLD_EXPOSURE', 'supplier', top.supplierId);
  const checkpoint = await reportIssueCheckpoint(userId, issueKey, {
    isPresent: true,
    direction: null, // we don't (yet) have a real prior-materiality comparison to call WORSE/BETTER honestly — STABLE is the honest default on repeat calls
    detail: {
      supplierId: top.supplierId,
      supplierName: top.supplierName,
      eventId: chain.EVENT?.id || null,
      eventType: chain.EVENT?.event_type || null,
      materialityBand: widened.materiality ? widened.materiality.materialityBand : null,
      spendSharePct: widened.materiality?.spendShare?.sharePct ?? null,
    },
  });

  // Only DETECTED/WORSENING/RECURRED are genuinely "new or newly-worth-flagging"
  // states — STABLE/IMPROVING on a repeat call means this exact risk was
  // already surfaced and nothing changed, so it is not re-surfaced.
  const surfaceableStatuses = ['DETECTED', 'WORSENING', 'RECURRED'];
  if (checkpoint.skipped || !surfaceableStatuses.includes(checkpoint.status)) {
    return null;
  }

  const materialityBand = widened.materiality ? widened.materiality.materialityBand : 'UNKNOWN';
  const sharePct = widened.materiality?.spendShare?.sharePct;

  return {
    id: 'WORLD_EXPOSURE_SUPPLIER',
    whatChanged: `A real ${chain.EVENT?.event_type || 'world event'} matches a real, verified exposure for ${top.supplierName || top.supplierId} — a supplier representing ${sharePct != null ? sharePct + '%' : 'a materiality-unknown share'} of real tenant purchase spend (${materialityBand} materiality).`,
    whyItMatters: materialityBand === 'HIGH'
      ? 'This supplier is a substantial-to-sole source of real spend, so a real disruption near it has an outsized, hard-to-diversify-around effect.'
      : 'This supplier carries a real, non-trivial share of spend, so a real disruption near it is worth tracking even though it is not the dominant source.',
    connectsTo: [
      { source: 'worldEventConsequence.widenSupplierUncertaintyFromEvent', field: 'chains' },
      { source: 'exposureMap.computeSupplierExposureMateriality', field: 'materiality' },
    ],
    mayHappen: 'Operational lead-time uncertainty for this supplier is qualitatively widened; no specific delay duration or shipment impact is asserted (product/shipment linkage data is honestly insufficient for this tenant).',
    whatToDo: 'Confirm whether this supplier has an active shipment/order in transit right now, and whether an alternate source exists outside the data captured here.',
    howSure: `EXPOSED — real business_exposure row matched to a real world_events row; issue status=${checkpoint.status}`,
    confirmationSignal: 'This risk is confirmed if the matched world event is followed by an actual delay, price change, or communication from this supplier.',
    invalidationSignal: 'This risk is invalidated if the world event resolves (effective_to passes, or a follow-up event marks the hazard over) without any operational effect, or if the exposure is later corrected/rejected as inaccurate.',
    evidence: { chain, materiality: widened.materiality, issueStatus: checkpoint.status, issueKey },
  };
}
...
async function buildMorningRevelationsV2(userId) {
  ...
  let worldExposure = null;
  try {
    const supplierConcentration = await getSupplierConcentration(userId).catch(e => ({ insufficientData: true, reason: e.message }));
    worldExposure = await revelationFromWorldExposure(userId, supplierConcentration);
  } catch (e) {
    safeLog('warn', '[RevelationEngine v2] world exposure revelation failed', { userId, error: e.message });
  }

  const combined = [...base.revelations, ...quiet, ...(worldExposure ? [worldExposure] : [])].slice(0, MAX_REVELATIONS);
  return {
    userId,
    ...
    worldExposureSurfaced: !!worldExposure,
    generatedAt: new Date().toISOString(),
  };
}

module.exports = { buildMorningRevelations, buildMorningRevelationsV2, findQuietTrendRevelations, revelationFromWorldExposure };
