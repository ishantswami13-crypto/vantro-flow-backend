...
async function verifySignalOutcomes(userId, signalId) {
  ...
  const predRes = await pool.query(
    `SELECT * FROM predictions
     WHERE user_id = $1 AND target = 'stockout_within_horizon' AND evaluation_status IS DISTINCT FROM 'RESOLVED'`,
    [userId]
  );
  ...
  for (const p of relevant) {
    ...
    if (horizonDate > now) {
      stillAwaiting.push({ predictionId: p.id, horizonDays: p.horizon_days, horizonDate: horizonDate.toISOString() });
      continue;
    }
    const actual = await observeActualStockoutState(userId, p.entity_id);
    if (actual == null) continue; // component no longer exists — cannot observe, cannot resolve
    const result = await resolvePrediction(p.id, actual);
    resolved.push({ ...result, horizonDays: p.horizon_days });
  }
  ...
  if (resolved.length > 0 && stillAwaiting.length === 0) {
    const anyStockout = resolved.some((r) => r.actualValue === 1);
    const outcome = anyStockout ? 'ineffective' : 'effective';
    for (const action of actionsRes.rows) {
      const notes = anyStockout
        ? `Verified against real inventory data: a stockout occurred despite this action (resolved horizons: ${resolved.map((r) => r.horizonDays).join(', ')}d).`
        : `Verified against real inventory data: no stockout occurred through the resolved horizon(s) (${resolved.map((r) => r.horizonDays).join(', ')}d).`;
      await pool.query(
...
module.exports = { observeActualStockoutState, verifySignalOutcomes };
