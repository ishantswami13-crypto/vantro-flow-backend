...
async function verifySignalOutcomes(userId, signalId) {
  ...
  // superseded_by_id IS NULL: only the current live head of each
  // (entity, target, horizon) chain is a candidate for resolution — an
  // older, superseded prediction is a preserved historical fact, not
  // something to re-verify (see predictionVersioning.js).
  const predRes = await pool.query(
    `SELECT * FROM predictions
     WHERE user_id = $1 AND target = ANY($2) AND evaluation_status IS DISTINCT FROM 'RESOLVED' AND superseded_by_id IS NULL`,
  ...
  if (stockoutResolved.length > 0 && stockoutAwaiting.length === 0) {
    ...
    for (const action of actionsRes.rows) {
      ...
      // Structured expected-vs-observed record (migration 045). Additive:
      // the flat outcome/outcome_at/outcome_notes columns above are left
      // exactly as before for existing readers — this is the richer record
      // the "close the loop" mission requires ("do not reduce outcome
      // verification to one text string"). expected_value 0 mirrors this
      // action's own expected_effect.expected_value (stockout avoided = the
      // shortfall this action targeted is fully closed); observed_value is
      // the real observed stockout flag (0 = no stockout, 1 = stocked out)
      // for the worst (most recent/severe) resolved horizon.
      const worst = stockoutResolved.reduce((w, r) => (w == null || r.actualValue > w.actualValue ? r : w), null);
      await pool.query(
        `INSERT INTO action_outcomes (user_id, action_id, verification_type, expected_metric, expected_value, observed_metric, observed_value, status, evidence, verified_at)
         VALUES ($1,$2,'stockout_within_horizon','stockout_occurred',0,'stockout_occurred',$3,$4,$5,NOW())`,
        [
          userId,
          action.id,
          worst ? worst.actualValue : null,
          anyStockout ? 'NOT_MET' : 'MET',
          JSON.stringify({ resolvedHorizons: stockoutResolved.map((r) => ({ horizonDays: r.horizonDays, actualValue: r.actualValue, predictionId: r.predictionId })) }),
        ]
      );
    }
  }
