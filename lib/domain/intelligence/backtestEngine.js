...
function rollingOriginBacktest(series, modelFn, opts = {}) {
  ...
  const errors = [];
  for (let cutoffIdx = minTrainSize - 1; cutoffIdx <= series.length - 1 - horizonSteps; cutoffIdx++) {
    // LEAKAGE BOUNDARY: trainPoints contains ONLY series[0..cutoffIdx] — nothing after it.
    const trainPoints = series.slice(0, cutoffIdx + 1);
    const actualPoint = series[cutoffIdx + horizonSteps];
  ...
  return { insufficientData: false, n, mae, rmse, wape, bias, coverageRate, errors };
}
