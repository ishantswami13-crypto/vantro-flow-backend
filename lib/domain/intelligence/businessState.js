...
async function loadBusinessState(supabase, userId) {
  const [actionsResult, cashflowResult, brainResult, externalConditionsResult, possibleFuturesResult, forecastSummaryResult] = await Promise.allSettled([
    loadRankedActions(supabase, userId),
    loadCashflow(userId),
    loadBrain(supabase, userId),
    loadExternalConditions(userId),
    loadPossibleFutures(userId),
  ...
  // Day 3 Part 15 — additive only. possibleFutures is a NEW field; no
  // existing field above is touched. Its own failure never affects the
  // rest of Business State (Promise.allSettled + explicit fallback below),
  // and it is always clearly labeled BASELINE — never presented as an
  // observed/current fact the way overallState/rankedActions are.
  const possibleFutures = possibleFuturesResult.status === 'fulfilled'
    ? possibleFuturesResult.value
    : { status: 'UNAVAILABLE', reason: 'possibleFutures lookup failed', error: 'unavailable' };
  if (possibleFuturesResult.status === 'rejected') {
    safeLog('warn', '[BusinessState] possibleFutures failed', { userId, error: possibleFuturesResult.reason?.message });
  }

  // Day 4 Part 14 — additive only. forecastSummary is a NEW field; no
  // existing field above is touched (verify via git diff — only this
  // addition and the new Promise.allSettled slot change this file). Gated
  // by the forecast's own evidence-sufficiency status, mirroring the Day 3
  // possibleFutures gating pattern above — never presented as an observed
  // fact when the underlying forecast is UNSUPPORTED/insufficient.
  const forecastSummary = forecastSummaryResult.status === 'fulfilled'
    ? forecastSummaryResult.value
    : { status: 'UNAVAILABLE', reason: 'forecastSummary lookup failed', error: 'unavailable' };
  if (forecastSummaryResult.status === 'rejected') {
    safeLog('warn', '[BusinessState] forecastSummary failed', { userId, error: forecastSummaryResult.reason?.message });
  }
  ...
  return {
    ...
    sections: {
      ...
      possibleFutures: possibleFuturesResult.status === 'fulfilled' ? 'ok' : 'error',
      forecastSummary: forecastSummaryResult.status === 'fulfilled' ? 'ok' : 'error',
    },
...
/**
 * Day 4 Part 14 — reads forecastEngine.forecast() for cash_position only
 * (the one end-to-end-implemented target). Never computes anything new;
 * purely composes the existing forecast contract output.
 */
async function loadForecastSummary(userId) {
  const { forecast } = require('./forecastEngine');
  const result = await forecast({ target: 'cash_position', entity_type: 'tenant_cash', entity_id: userId, horizon: 30, as_of: new Date().toISOString() });
  return {
    status: result.status || (result.point_estimate != null ? 'FORECASTED' : 'INSUFFICIENT_CONTEXT'),
    target: result.target,
    point_estimate: result.point_estimate ?? null,
    interval: result.interval ?? null,
    model: result.model_name || result.model_selected || null,
...
// Day 3 Part 15 — thin, additive wrapper around cashConsequenceEngine.js's
// BASELINE cash consequence. Never a new computation; composes what Day 3
// already built. Always labeled BASELINE via cashConsequenceEngine's own
// `projection.kind` field — this function never adds a scenario here.
async function loadPossibleFutures(userId) {
  const { buildCashConsequence } = require('./cashConsequenceEngine');
  const cashConsequence = await buildCashConsequence(userId);
  return {
    status: cashConsequence.status,
    cashConsequence: cashConsequence.status === 'PROJECTED' ? cashConsequence : null,
    reason: cashConsequence.status !== 'PROJECTED' ? cashConsequence.reason : undefined,
  };
}
...
async function loadRankedActions(supabase, userId) {
  ...
  // Day 7 — additive-only enrichment. For any row that already references a
  // customer, optionally attach the cash-risk narrative (Day 7 Part 3) and a
  // priorityScoreV2 (Part 6) when the underlying data honestly supports it.
  // This NEVER changes priority/risk_level/sort order above, and never
  // throws — a failure here degrades to the row being left exactly as it
  // was (no `cashRiskNarrative`/`priorityScoreV2` field), matching this
  // file's existing fail-safe convention.
  await enrichRowsWithDay7Intelligence(rows, userId);

  return rows;
}

async function enrichRowsWithDay7Intelligence(rows, userId) {
  const { buildCashRiskNarrative } = require('./cashRiskNarrative');
  const { computePriorityScore } = require('./priorityScoring');

  for (const row of rows) {
    if (!row.customer?.id) continue;
    try {
      const narrative = await buildCashRiskNarrative({ userId, customerId: row.customer.id });
      if (narrative && !narrative.insufficientEvidence) {
        row.cashRiskNarrative = narrative;
        const scoreInput = {
          priority: row.priority,
          risk_level: row.risk_level,
          trajectory: deriveTrajectoryLabel(narrative),
          concentrationSharePct: narrative.evidence?.find(e => e.type === 'revenue_concentration')?.sharePct,
          confidenceComponents: narrative.confidence_components,
        };
        const scored = computePriorityScore(scoreInput);
        if (scored) row.priorityScoreV2 = scored;
      }
    } catch (err) {
      safeLog('warn', '[BusinessState] Day7 cash-risk enrichment failed', { userId, customerId: row.customer.id, error: err.message });
    }
  }
}

function deriveTrajectoryLabel(narrative) {
  // cashRiskNarrative.js now returns a structured `trajectory` field
  // directly (UNKNOWN/DETERIORATING/IMPROVING/STABLE) — use that instead of
  // regexing the human-readable `observation` prose for substrings.
  return narrative.trajectory && narrative.trajectory !== 'UNKNOWN' ? narrative.trajectory : undefined;
}
