...
function deriveTrajectoryLabel(narrative) {
  const obs = narrative.observation || '';
  if (obs.includes('deteriorating')) return 'DETERIORATING';
  if (obs.includes('improving')) return 'IMPROVING';
  if (obs.includes('stable')) return 'STABLE';
  return undefined;
}
