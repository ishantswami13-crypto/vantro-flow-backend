...
// --- WORLD INTELLIGENCE EXPANSION addition (Part 8, additive) ---
// Pure formatter: given the pieces of an already-computed external chain
// (FX or hazard), renders the explicit
// external signal -> exposure -> variable -> adjustment -> downstream forecast
// trace the mission asks for. Does not query the DB itself and does not
// replace getForecastLineage — it composes with whatever chain object
// fxScenarioEngine.js / worldEventConsequence.js already returned, so it
// stays honest about only describing edges that were actually computed.
function describeExternalChain({ signal, exposure, variable, adjustment, downstreamForecastId = null }) {
  const stages = [
    { stage: 'external_signal', present: !!signal, detail: signal || null },
    { stage: 'exposure', present: !!exposure, detail: exposure || null },
    { stage: 'affected_variable', present: !!variable, detail: variable || null },
    { stage: 'adjustment', present: !!adjustment, detail: adjustment || null },
    { stage: 'downstream_forecast', present: !!downstreamForecastId, detail: downstreamForecastId },
  ];
  const complete = stages.every(s => s.present);
  const firstGapIndex = stages.findIndex(s => !s.present);
  return {
    stages,
    complete,
    stoppedAt: complete ? null : stages[firstGapIndex].stage,
    note: complete
      ? 'Full external-signal-to-forecast trace: every stage has a real (or explicitly seeded) value.'
      : `Chain honestly stops at '${stages[firstGapIndex].stage}' — no fabricated downstream edge is added past the last real/seeded stage.`,
  };
}

module.exports = { getForecastLineage, MAX_LINEAGE_DEPTH, describeExternalChain };
