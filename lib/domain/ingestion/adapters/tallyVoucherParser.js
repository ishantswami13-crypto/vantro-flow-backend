...
function parseVouchers(xml) {
  const vouchers = [];
  // CONFIRMED against a real TallyPrime response (2026-09-10): Tally's own
  // CMPINFO header contains a plain `<VOUCHER>2</VOUCHER>` COUNT field
  // (not a voucher record) that a bare `<VOUCHER\b...>` match would
  // wrongly treat as an empty voucher block. Real voucher records always
  // carry a VCHTYPE attribute on the opening tag — require it.
  const blocks = xml.match(/<VOUCHER\s+[^>]*VCHTYPE="[^"]*"[^>]*>[\s\S]*?<\/VOUCHER>/gi) || [];
