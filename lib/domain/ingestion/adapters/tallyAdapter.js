...
// (2026-09-10): a "Day Book" TYPE=Data/ID=Day Book report request (the
// original implementation) returns HTTP 200 with only company metadata,
// zero voucher entries, even when real vouchers exist in range — same
// class of bug found and fixed in scripts/tally/diagnose.js's "List of
// Companies" request. A TYPE=Collection request against a real TDL-defined
// Voucher collection is what this Tally build actually honors, and it
// returns full voucher data including genuine stable identifiers (GUID,
// MASTERID, VOUCHERKEY) that a Day Book export never carries.
function dayBookXML(fromDate, toDate, company) {
 ...
 <BODY>
  <DESC>
   ...
   <TDL>
    <TDLMESSAGE>
     <COLLECTION NAME="Starlane Vouchers" ISMODIFY="No">
      <TYPE>Voucher</TYPE>
      <FETCH>DATE,VOUCHERTYPENAME,VOUCHERNUMBER,PARTYLEDGERNAME,NARRATION,AMOUNT,ALLINVENTORYENTRIES.LIST,ALLLEDGERENTRIES.LIST,GUID,MASTERID</FETCH>
     </COLLECTION>
    </TDLMESSAGE>
   </TDL>
  </DESC>
