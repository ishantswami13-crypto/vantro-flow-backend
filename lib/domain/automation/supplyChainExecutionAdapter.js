...
async function executeSupplyChainAction(userId, action) {
  ...
  // Idempotency at the adapter boundary itself — not just in the calling
  // route. Any caller (a retried HTTP request, a future non-HTTP scheduler)
  // that reaches this function twice for the same action must get back the
  // SAME execution, never a second real purchase_orders row.
  const priorExec = await pool.query(
    `SELECT * FROM execution_records WHERE user_id = $1 AND ai_action_id = $2 ORDER BY created_at ASC LIMIT 1`,
    [userId, action.id]
  );
  if (priorExec.rows.length > 0) {
    const priorPoRes = await pool.query(`SELECT * FROM purchase_orders WHERE related_ai_action_id = $1 LIMIT 1`, [action.id]);
    return {
      mode,
      liveExternalWriteOccurred: false,
      alreadyExecuted: true,
      purchaseOrder: priorPoRes.rows[0] || null,
      executionRecord: priorExec.rows[0],
      note: 'This action was already executed — returning the existing execution receipt rather than creating a duplicate.',
    };
  }
  ...
  // parameters (migration 045) is the exact, frozen payload the user saw and
  // approved — never re-derived or recomputed here. If it's absent (an
  // older action created before this column existed), fall back to the
  // reason_json-derived shape so old pending actions don't hard-fail.
  const params = action.parameters || null;
  ...
  if (mode === 'DEMO_ADAPTER') {
    const poRes = await pool.query(
      ...
      [
        userId,
        params ? params.supplier.name : (supplier ? supplier.name : 'Unknown supplier'),
        params ? params.supplier.phone : (supplier ? supplier.phone : null),
        JSON.stringify(params
          ? { products: params.products, currency: params.currency, delivery_target_days: params.delivery_target_days, notes: params.notes, note: 'Created by Starlane demo ERP adapter — no live external system call was made. Executed exactly the parameters shown for approval.' }
          : { component_id: component ? component.id : null, component_name: component ? component.name : null, note: 'Created by Starlane demo ERP adapter — no live external system call was made.' }),
